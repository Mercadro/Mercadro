    ...
    def import_order_process_by_remote_ids(self, instance, order_ids):
        ...
        if order_ids:
            ...
            if len(order_ids.split(',')) <= 50:
                ...
                results = shopify.Order().find(ids=','.join(order_ids_list))
                if results:
                    _logger.info('Length of Shopify orders %s import from instance name: %s' % (
            ...
            if results:
                ...
                if order_data_queue:
                    ...
                    _logger.info("Imported order queue : {0} of Instance : {1} via Import shopify Remote Order ids Successfully".format(
