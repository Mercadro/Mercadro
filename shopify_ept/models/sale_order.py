...
class SaleOrder(models.Model):
    ...
    def update_shopify_order(self, queue_line, log_book):
        ...
        # Below condition use for, In shopify store there is cancel order with full refund.
        if shopify_status == "refunded" and order_data.get('cancel_reason'):
            ...
            if not cancelled:
                message = "System can not cancel the order {0} as one of the picking is in the done state.".format(
                    order.name)
        # Below condition use for, In shopify store there is full refund but not cancel.
        elif shopify_status == "refunded" and not order_data.get('cancel_reason'):
            total_refund = 0.0
