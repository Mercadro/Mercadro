        ...
        if not state:
            ...
            if response:
                ...
                if not country:
                    ...
                    country = country_obj.search([('name', 'ilike', response.get('country')), (
                        'code', 'ilike', response.get('country abbreviation'))])
                if not country:
                    country = country_obj.create({'name': response.get('country'),
                                                  'code': response.get('country abbreviation')})
                state = self.env['res.country.state'].search(
                    ['|', ('name', 'ilike', response.get('places')[0].get('state')), ('code', 'ilike', response.get('places')[0].get('state abbreviation')),
                     ('country_id', '=', country.id)], limit=1)
                if not state:
